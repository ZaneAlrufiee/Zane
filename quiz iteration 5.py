...
class QuizGUI:
    ...
    def __init__(self, questions):
        ...
        self.radio_buttons = []
        for i in range(4):
            radio_button = tk.Radiobutton(self.root, variable=self.radio_var, value=i, font=("Arial", 10))
            self.radio_buttons.append(radio_button)
            radio_button.pack()

        # Create the Skip and Next buttons
        self.skip_button = tk.Button(self.root, text="Skip", command=self.skip_question, font=("Arial", 10))
        self.skip_button.pack(side=tk.LEFT, padx=10, pady=20)

        self.next_button = tk.Button(self.root, text="Next", command=self.next_question, font=("Arial", 10))
        self.next_button.pack(side=tk.RIGHT, padx=10, pady=20)

        # Create the score label
        self.score_label = tk.Label(self.root, text="Score: 0", font=("Arial", 10))
